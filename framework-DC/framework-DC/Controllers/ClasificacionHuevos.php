...
class ClasificacionHuevos extends Controllers
{
    ...
    public function getClasificaciones()
    {
        try {
            ...
            json_response(['status' => false, 'msg' => 'Error: ' . $e->getMessage()], 500);
        }
    }
    ...
    public function setClasificacion()
    {
        try {
            ...
            $request = $this->model->insertClasificacion(
                $intLoteId, $strFecha, $intJumbo, $intAAA, $intAA, 
                $intA, $intB, $intC, $intRotos, $strObservaciones, $intRegistradoPor
            );

            if ($request > 0) {
                json_response(['status' => true, 'msg' => 'Clasificación creada exitosamente', 'id' => $request], 201);
            } else {
    ...
    public function putClasificacion($id)
    {
        try {
            ...
            if ($intId <= 0 || $intLoteId <= 0) {
                json_response(['status' => false, 'msg' => 'Datos incompletos'], 400);
                return;
            }

            $request = $this->model->updateClasificacion(
                $intId, $intLoteId, $strFecha, $intJumbo, $intAAA, 
                $intAA, $intA, $intB, $intC, $intRotos, $strObservaciones
            );

            if ($request) {
                json_response(['status' => true, 'msg' => 'Clasificación actualizada exitosamente'], 200);
            } else {
                json_response(['status' => false, 'msg' => 'Error al actualizar'], 500);
            }
        } catch (Exception $e) {
            json_response(['status' => false, 'msg' => 'Error: ' . $e->getMessage()], 500);
        }
        ...
        try {
            ...
            if ($intId > 0) {
                ...
                if ($request) {
                    ...
                    json_response(['status' => false, 'msg' => 'Error al eliminar'], 500);
                }
            } else {
                json_response(['status' => false, 'msg' => 'ID inválido'], 400);
            }
...
?>
'@ | Out-File -FilePath "Controllers/ClasificacionHuevos.php" -Encoding UTF8 -Force

Write-Host "✅ Controlador ClasificacionHuevos.php creado" -ForegroundColor Green